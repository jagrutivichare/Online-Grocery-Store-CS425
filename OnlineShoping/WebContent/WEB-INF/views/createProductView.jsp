...
<body>
	...
	<form method="POST" enctype="multipart/form-data"
		action="doCreateProduct">
		<table border="0">
			<tr>
				<td>Product ID</td>
				<td><input type="text" name="pid" value="${product.productID}" /></td>
			</tr>
			<tr>
				<td>Name</td>
			...
			<tr>
				...
				<td><select name=prodtype>
						...
						<option ${product.producttType == 'Vegetables' ? 'selected' : ''}>Vegetables</option>
						<option ${product.producttType == 'Milk/Dairy' ? 'selected' : ''}>Milk/Dairy</option>
						<option ${product.producttType == 'Beverages' ? 'selected' : ''}>Beverages</option>
				</select></td>
			</tr>
			<tr>
				<td>Price($)</td>
			...
			<tr>
				...
				<td><select name=supplier>
						<c:forEach items="${supplierList}" var="supplier">
							<option ${product.supplier == supplier.supplierID ? 'selected' : ''} value="${supplier.supplierID}">${supplier.supplierName}</option>
						</c:forEach>
			...
			<tr>
				...
				<td><select name=warehouse>
						<c:forEach items="${warehouseList}" var="warehouse">
							<option ${product.warehouse == warehouse.warehouseName? 'selected' : ''} value="${warehouse.warehouseName}">${warehouse.warehouseName}</option>
						</c:forEach>
